...
export default function CollegeDisplay() {
    ...
    const performSearch = useCallback(async (query: string) => {
        ...
        try {
            ...
            // Handle response structure - ApiResponse wraps data in response.data
            let results: SearchResults | null = null;
            if (response.data) {
                results = response.data;
            } else {
                // Fallback: check if response itself is SearchResults (shouldn't happen but just in case)
                const responseAny = response as any;
                if (responseAny.colleges || responseAny.courses || responseAny.exams || responseAny.careers) {
                    results = responseAny as SearchResults;
                }
            }
                                ...
                                {isSearching ? (
                                    ...
                                    <div className="p-2">
                                                ...
                                                {searchResults.colleges.map((college) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <p className="text-xs text-gray-500 truncate">{college.city}, {college.state}</p>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        {searchResults.courses.length > 0 && (
                                            <div className="mb-4">
                                                <h4 className="text-xs font-semibold text-gray-500 uppercase px-3 py-2">Courses</h4>
                                                {searchResults.courses.map((course) => (
                                                    <button
                                                        key={course.id}
                                                        onClick={() => handleSearchResultClick('course', course.id)}
                                                        className="w-full text-left px-3 py-2 hover:bg-gray-50 rounded-lg transition-colors flex items-center gap-3"
                                                    >
                                                        <BookOpen className="w-4 h-4 text-purple-500 flex-shrink-0" />
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-medium text-gray-900 truncate">{course.title}</p>
                                                            <p className="text-xs text-gray-500 truncate">{course.level} • {course.durationMonths} months</p>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        {searchResults.exams.length > 0 && (
                                            <div className="mb-4">
                                                <h4 className="text-xs font-semibold text-gray-500 uppercase px-3 py-2">Exams</h4>
                                                {searchResults.exams.map((exam) => (
                                                    <button
                                                        key={exam.id}
                                                        onClick={() => handleSearchResultClick('exam', exam.id)}
                                                        className="w-full text-left px-3 py-2 hover:bg-gray-50 rounded-lg transition-colors flex items-center gap-3"
                                                    >
                                                        <FileText className="w-4 h-4 text-green-500 flex-shrink-0" />
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-medium text-gray-900 truncate">{exam.name}</p>
                                                            <p className="text-xs text-gray-500 truncate">{exam.examType} • {exam.examDate ? new Date(exam.examDate).toLocaleDateString() : 'TBA'}</p>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        {searchResults.careers.length > 0 && (
                                            <div className="mb-4">
                                                <h4 className="text-xs font-semibold text-gray-500 uppercase px-3 py-2">Careers</h4>
                                                {searchResults.careers.map((career) => (
                                                    <button
                                                        key={career.id}
                                                        onClick={() => handleSearchResultClick('career', career.id)}
                                                        className="w-full text-left px-3 py-2 hover:bg-gray-50 rounded-lg transition-colors flex items-center gap-3"
                                                    >
                                                        <Briefcase className="w-4 h-4 text-orange-500 flex-shrink-0" />
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-medium text-gray-900 truncate">{career.title}</p>
                                                            {career.avgSalary && (
                                                                <p className="text-xs text-gray-500 truncate">Avg. Salary: ₹{career.avgSalary.toLocaleString()}/year</p>
                                                            )}
                                                        </div>
                                                    </button>
                                                ))}
