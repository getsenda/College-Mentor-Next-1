...
import { ChevronDown, Search, User, Menu, X, LogIn, UserPlus, BookOpen, GraduationCap, Award, Briefcase, Globe, Calculator, Target, TrendingUp, ChevronRight, Building2, GraduationCap as CourseIcon, FileText, Briefcase as CareerIcon, Loader2, Users, Bell, Info, DollarSign, Plane } from 'lucide-react';
...
const PrimaryNavigation = () => {
  ...
  const isNavActive = (itemName: string): boolean => {
    ...
    if (itemName === 'Colleges' && (pathname.startsWith('/colleges') || pathname.startsWith('/college-compare'))) return true;
  ...
  const handleSearchChange = useCallback((value: string) => {
    ...
    // Only search if query is at least 1 character (removed minimum length requirement for testing)
  ...
  const isCollegeComparePage = pathname?.startsWith('/college-compare');
  const isRankPredictorAny = pathname?.startsWith('/rank-predictor');
  const isCourseComparePage = pathname?.startsWith('/course-compare');
  const isCareerDetailPage = pathname?.includes('/careers/interests') || pathname?.includes('/careers/recommendations');
  const isNewsPage = pathname?.startsWith('/news');
  const isExamsListPage = pathname?.startsWith('/exams/list');
  const isExamDetailPage = pathname?.startsWith('/exams/') && !pathname?.startsWith('/exams/list');
  const isCollegeDetailsPage = pathname?.startsWith('/college-details');
  const isCollegesListPage = pathname?.startsWith('/colleges/list');
  const isAuthPage = pathname?.startsWith('/auth');
  const isCareerAssessmentPage = pathname?.startsWith('/career-assessment');
  // College predictor dedicated sub-pages (form & results) are on light backgrounds,
  // so use dark logo/nav styles instead of the default "white on transparent".
  const isCollegePredictorSubPage =
    pathname?.startsWith('/college-predictor/form') ||
    pathname?.startsWith('/college-predictor/results');
  // Pages that use light nav bar with black text and colored logo
  const hasLightNav = isCareerDetailPage || isCollegePredictorSubPage || isNewsPage || isExamsListPage || isExamDetailPage || isCollegeDetailsPage || isCollegesListPage || isAuthPage || isRankPredictorAny || isCareerAssessmentPage;
  const desktopLogoColorClasses = hasLightNav
    ? "brightness-100 invert-0"           // colored logo on light nav pages
    : isCollegeComparePage || isCourseComparePage
      ? "brightness-100 invert-0"           // show original logo
      : (!isScrolled && isCoursesPage)
  ...
  // const desktopLogoColorClasses = (!isScrolled && isCoursesPage )
  //   ? 'brightness-100 invert-0'
  //   : isScrolled
  //     ? 'brightness-100 invert-0'
  //     : 'brightness-0 invert';
  ...
  //old one
  // const desktopNavItemStateClasses = (!isScrolled && isCoursesPage)
  //   ? 'text-blue-600 hover:text-blue-700 hover:bg-blue-50 hover:ring-2 hover:ring-blue-100'
  //   : isScrolled
  ...
  const desktopNavItemStateClasses = hasLightNav
    ? 'text-slate-900 hover:text-slate-900 hover:bg-gray-100 hover:shadow-gray-200 hover:ring-2 hover:ring-gray-200'
    : isCollegeComparePage || isCourseComparePage
      ? (
        isScrolled
          ? 'text-gray-700 hover:text-gray-900 hover:bg-gray-100 hover:ring-2 hover:ring-gray-200'
          : 'text-primary hover:text-[#1a5fc4] hover:bg-blue-50 hover:ring-2 hover:ring-blue-100'
      )
      : (!isScrolled && isCoursesPage)
        ? 'text-blue-600 hover:text-blue-700 hover:bg-blue-50 hover:ring-2 hover:ring-blue-100'
        : isScrolled
  ...
  const searchButtonBaseClasses = 'p-2 rounded-full border transition-colors duration-200';
  const searchButtonVariantClasses = hasLightNav
  ...
  // const desktopSearchIconClasses = (!isScrolled && isCoursesPage) ? 'text-blue-600' : 'text-white';
  const desktopSearchIconClasses = hasLightNav
    ? "text-slate-700"
    : isCollegeComparePage || isCourseComparePage
      ? "text-green-600"               // override only for college compare
      : (!isScrolled && isCoursesPage)
        ? "text-blue-600"
        : "text-white";
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[300] pointer-events-none">
            ...
            {!isSearching && hasSearched && searchResults && (
              <div className="space-y-6">
                ...
                {searchResults.courses && searchResults.courses.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <CourseIcon className="text-primary" size={18} />
                ...
                {searchResults.careers && searchResults.careers.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <CareerIcon className="text-primary" size={18} />
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        logger.log('router.push to:', link);
                        setActiveDropdown(null);
                      }}
                    >
      ...
      >
          ...
          <div className="flex items-center justify-between py-5 sm:py-6">
            ...
            <div className="flex items-center space-x-3">
              <Button
                ...
                className="hover:bg-primary/10 transition-colors duration-300 text-gray-700"
              >
            ...
            <div className="flex items-center space-x-2">
              <Button
                ...
                className="hover:bg-primary/10 transition-colors duration-300 text-gray-700 sm:hidden"
              >
                <Search size={18} />
              </Button>
            </div>
        ...
        {mobileMenuOpen && (
          <>
            ...
            {/* Slide-out Menu - Fullscreen */}
            <div className="fixed top-0 left-0 right-0 bottom-0 w-screen h-screen bg-white z-50 shadow-xl animate-slide-in-right overflow-y-auto">
              {/* Header */}
                        ...
                        {item.name === 'Colleges' && collegedekhoDropdownMenus.Colleges && (
                          <div className="p-3 space-y-3 max-h-[70vh] overflow-y-auto">
                            ...
                            <div className="border-t pt-3">
                              ...
                              <div className="space-y-0.5">
                                {collegedekhoDropdownMenus.Colleges.streams[selectedStream]?.exams?.map((exam, index) => (
                                  <button
                                    key={index}
                                    className="w-full text-left px-2 py-1.5 text-xs text-gray-600 hover:text-primary hover:bg-gray-50 rounded transition-colors"
      ...
      {/* Primary Navigation - ONLY THIS, NO SECONDARY */}
      <nav
        ref={primaryNavRef}
        className={`transition-all duration-300 relative pointer-events-auto ${isCareerAssessmentPage
          ? 'bg-gradient-to-r from-[#F0F9FF] via-[#F0F9FF] to-[#E0F2FE]'
          : (isScrolled || hasLightNav
            ? 'bg-gradient-to-r from-white via-gray-50 to-white border-b border-slate-100'
            : 'bg-transparent')
          }`}
        onMouseLeave={handleMouseLeave}
        style={{
          backgroundColor: isCareerAssessmentPage
            ? (activeDropdown === 'search' ? '#F0F9FF' : 'transparent')
            : (activeDropdown === 'search'
              ? (isScrolled || hasLightNav ? 'rgba(255, 255, 255, 1)' : 'transparent')
              : (isScrolled || hasLightNav ? 'rgba(255, 255, 255, 0.98)' : 'transparent')),
          backdropFilter: activeDropdown === 'search'
            ? 'none'
            : (isCareerAssessmentPage ? 'none' : (isScrolled || hasLightNav ? 'blur(8px)' : 'none'))
        }}
      >
        <div className="container mx-auto px-4 max-w-7xl">
              ...
              <div className="flex items-center space-x-8">
                ...
                <Link href="/" className="flex items-center cursor-pointer">
                  <img
                    src="/lovable-uploads/f76258ce-d7f2-42ce-a5a8-490fc7ec7425.png"
                    alt="College Mentor Logo"
                    className={`h-14 w-auto transition-all duration-300 ${desktopLogoColorClasses}`}
                    onError={(e) => {
                    ...
                    <div key={item.name} className="relative">
                      <button
                        className={`group flex items-center space-x-1 px-4 py-3 rounded-lg relative whitespace-nowrap hover:scale-105 hover:-translate-y-0.5 hover:z-10 transition-[color,font-weight,font-size,transform,background-color] duration-300 ease-out ${isNavActive(item.name)
                          ? `${(isScrolled || isCareerAssessmentPage) ? 'text-secondary' : 'text-[#FFC206]'} font-bold text-base scale-105 text-xl`

                          : `text-sm font-medium ${desktopNavItemStateClasses}`
                          }`}
                      ...
                      <span
                        className={`absolute bottom-1 left-[45%] -translate-x-1/2 w-2/3 h-0.5 rounded-full origin-center transition-transform duration-300 ease-out ${isNavActive(item.name)
                          ? (isScrolled || isCareerAssessmentPage)
              ...
              <div className="flex items-center space-x-10">
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`${searchButtonBaseClasses} ${searchButtonVariantClasses}`}
                  >
                    <Search className={desktopSearchIconClasses} size={18} />
                  </button>

                  {/* Search Dropdown */}
                  {activeDropdown === "search" && (
                    <>
                      {/* Blurred Background Overlay - Starts below header */}
                      <div
                        className="fixed left-0 right-0 bottom-0 bg-black/20 backdrop-blur-sm z-[240]"
                        onClick={() => setActiveDropdown(null)}
      ...
      ${isScrolled ? "bg-secondary hover:bg-[#00a77e]" : "bg-white text-secondary hover:bg-[#00a77e] hover:text-white"}
    `}
                >
        ...
        {activeDropdown && activeDropdown !== 'search' && (collegedekhoDropdownMenus as Record<string, any>)[activeDropdown] && (
          <div className="absolute top-full left-0 right-0 w-full z-[200] flex justify-center pointer-events-none">
            <div
              className="container mx-auto px-4 max-w-7xl pointer-events-auto"
              onMouseEnter={handleDropdownMouseEnter}
              onMouseLeave={handleMouseLeave}
            >
              <div className="bg-white border border-gray-200 rounded-b-xl shadow-2xl p-5 animate-in slide-in-from-top-2 duration-300 ease-out max-h-[80vh] overflow-y-auto flex flex-col">
                {/* Colleges Dropdown */}
                    ...
                    <div className="flex-1 grid grid-cols-3 gap-5 overflow-y-auto max-h-[560px]">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="space-y-0.5">
                          ...
                          <button onClick={() => { router.push('/colleges/list'); setActiveDropdown(null); }} className="block w-full text-left text-sm font-medium text-primary hover:text-primary/80 px-2 py-1.5">
                            View All →
                          </button>
                        </div>
                        ...
                        {collegedekhoDropdownMenus.Colleges.streams[selectedStream]?.byLocation && (
                          <div className="pt-2">
                            ...
                            <div className="space-y-0.5">
                              {collegedekhoDropdownMenus.Colleges.streams[selectedStream].exams.slice(0, 6).map((location, index) => (
                                <button
                                  key={index}
                              ...
                              <button onClick={() => { router.push('/exams'); setActiveDropdown(null); }} className="block w-full text-left text-sm font-medium text-primary hover:text-primary/80 px-2 py-1.5">
                                View All →
                              </button>
                            </div>
                          </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="space-y-0.5">
                          ...
                          <button onClick={() => { router.push('/courses'); setActiveDropdown(null); }} className="block w-full text-left text-sm font-medium text-primary hover:text-primary/80 px-2 py-1.5">
                            View All →
                          </button>
                        </div>
                        ...
                        {collegedekhoDropdownMenus.Colleges.streams[selectedStream]?.rankPredictor && (
                          <div className="pt-2">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <button onClick={() => { router.push('/rank-predictor'); setActiveDropdown(null); }} className="block w-full text-left text-sm font-medium text-primary hover:text-primary/80 px-2 py-1.5">
                                View All →
                              </button>
                            </div>
                          </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="space-y-0.5">
                          ...
                          <button onClick={() => { router.push('/college-predictor'); setActiveDropdown(null); }} className="block w-full text-left text-sm font-medium text-primary hover:text-primary/80 px-2 py-1.5">
                            View All →
                          </button>
                        </div>
                        ...
                        {collegedekhoDropdownMenus.Colleges.streams[selectedStream]?.compare && (
                          <div className="pt-2">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <button onClick={() => { router.push('/college-compare'); setActiveDropdown(null); }} className="block w-full text-left text-sm font-medium text-primary hover:text-primary/80 px-2 py-1.5">
                                View All →
                              </button>
                            </div>
                          </div>
                ...
                {activeDropdown === 'Careers' && (
                  <div className="flex gap-4 flex-1 min-h-0 overflow-hidden">
                    ...
                    <div className="w-72 border-r pr-3 space-y-0 overflow-y-auto flex-shrink-0 scrollbar-visible">
                      {collegedekhoDropdownMenus.Careers.streams.map((sector, index) => (
                        <button
                          key={sector.name}
                          onClick={() => setSelectedCareerSector(index)}
                ...
                {/* More Dropdown */}
                {activeDropdown === 'More' && (
                  <div className="grid grid-cols-3 gap-6 w-full overflow-y-auto scrollbar-hide">
                    {collegedekhoDropdownMenus.More.items.map((menuItem, index) => {
                      const IconComponent = menuItem.icon;
                      return (
                        <button
